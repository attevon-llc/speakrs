...
impl EmbeddingModel {
    ...
    ///
    /// All 10 ORT sessions (weights + arenas — the VRAM) are shared through
    ...
    /// (~130 MB host RAM), not N × engine. Handles may run concurrently — each
    ...
    pub fn clone_shared(&self) -> Result<Self, ort::Error> {
        Ok(Self {
            ...
            ort: OrtEmbeddingState {
                ...
                split_fbank_pool: self.ort.split_fbank_pool.clone(),
